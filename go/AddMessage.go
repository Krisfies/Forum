...
func AddMessage(mess Data_Message) (int64, error) {
	result, err := db.Exec("INSERT INTO data_message (Content, Author, Date, Topic_ID, Nbr_Like) VALUES (?, ?, ?, ?, ?)", mess.Content, mess.Author, mess.Date, mess.Topic_ID, mess.Nbr_Like)
	fmt.Println(result)
	if err != nil {
